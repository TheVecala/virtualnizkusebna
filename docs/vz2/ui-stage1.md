...
Změněné testy: `tests/vz2_content.browser.js`,
`tests/vz2_cutover.integration.js`, `tests/vz2_integration.test.js`;
nový `tests/vz2_layout.browser.js`. Nový tento audit/ověřovací dokument.

## Ověření 21. 9. 2026

- PHP lint `vz2.php`, JS syntaxe všech tří upravených aplikačních skriptů
  a `git diff --check`: bez chyb.
- `node tests/vz2_timestamps.test.js`: PASS.
- `node tests/multitrack_contract.test.js`: PASS, 45 DOM identifikátorů.
- `node tests/offline_audio_download_contract.test.js`: PASS.
- `node tests/vz2_integration.test.js` s `VZ2_TEST_BROWSER=1`: **179/179 PASS**.
