...
- `tests/vz2_integration.test.js` s `VZ2_TEST_BROWSER=1`: **90 PASS**. Privátní
...
- Nový integrační modul `tests/vz2_timestamps.integration.js`: práva, CSRF,
...
- `tests/vz2_timestamps.browser.js`: skutečné stránky a HTTP v headless Edge
...
- `tests/vz2_timestamps.test.js`: PASS — hranice intervalů, shodné body,
...
node tests/vz2_timestamps.test.js
```
...
Viz `deploy/vz2-stage3/README.cs.md`. Balíček obsahuje pouze devět provozních
