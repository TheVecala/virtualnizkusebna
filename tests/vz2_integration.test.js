...
`);
    ...
    async function login(name) {
        ...
        assert.match(r.headers.get('set-cookie') || '', /; HttpOnly/i, 'login cookie must be HttpOnly');
        assert.match(r.headers.get('set-cookie') || '', /; SameSite=Lax/i, 'login cookie must use SameSite=Lax');
    ...
    try {
        ...
        const logout = await api('guest', {action:'logout'});
        assert.match(logout.headers.get('set-cookie') || '', /; HttpOnly/i);
        assert.match(logout.headers.get('set-cookie') || '', /; SameSite=Lax/i);
        check(logout.status === 200 && (await api('guest')).status === 401, 'guest logout invalidates API session and preserves cookie flags');
        await login('guest');
