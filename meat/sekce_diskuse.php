...
// Název tabulky ze SESSION - escapovat
$aktualni_diskuse = $mysqli->real_escape_string($aktualni_diskuse);
...
if ($celkem > ROWS) {
    $offset  = $od - 1;
    $vysledek = $mysqli->query("SELECT cas, vzkaz, jmeno FROM `$aktualni_diskuse`
        ORDER BY cas DESC LIMIT $offset, " . ROWS);
} else {
    $vysledek = $mysqli->query("SELECT * FROM `$aktualni_diskuse` ORDER BY cas DESC");
}
?>
