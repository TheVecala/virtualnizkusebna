...
<!-- Formulář - oddělený ID aby nekolidoval s diskusí válu -->
<div class="comment-form">
  <form id="form_napady">
    <textarea id="napady_text" class="cf-textarea" rows="2" placeholder="napsat nápad..."></textarea>
    <div class="cf-row">
      <input id="napady_jmeno" class="cf-input" type="text" placeholder="jméno">
      <button type="submit" class="cf-btn">uložit</button>
    </div>
    <div id="napady_chyba" class="cf-chyba"></div>
  </form>
</div>
...
$(document).on('submit', '#form_napady', function(e) {
  ...
  $.post('/php/vlozit_komentar.php', {
    ...
    odkaz:      '',
    odkaz2:     '',
    pouzit_hlavni_diskusi: '1'   // příznak pro vlozit_komentar.php
  }, function(data) {
    if (data.ok) {
      nacistPanel('napady');
      $('#napady_text').val('');
