...
if( $_SESSION['vysledek'] == "vytvořeno"  ) {
	...
	$md5_nick = md5($nick);
	$cele_jmeno = mysql_real_escape_string($_POST['cele_jmeno']);
	// if cele jmeno = 0 then = nick
    $heslo = mysql_real_escape_string($_POST['heslo']);
    $over_heslo = mysql_real_escape_string($_POST['over_heslo']);
    $md5_heslo = md5($heslo);
    $email = mysql_real_escape_string($_POST['email']);
    $adresa_diskuse = 'diskuse_'. mysql_real_escape_string($_POST['nick']).'__'.$md5_nick ;
    ...
    if($nick==""){echo"Nebyl vyplněn nick!";}
        ...
        $sql= mysql_query("INSERT INTO uzivatele VALUES ('','$nick','$md5_heslo','$cele_jmeno ','$email','$md5_nick','$adresa_diskuse')") or die(mysql_error());
