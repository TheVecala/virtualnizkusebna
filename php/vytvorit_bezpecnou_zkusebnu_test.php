			...
			{  
        		if (mkdir($koren.$nick))									
					{ 
					    ...
					    if (mkdir($koren.$nick."/".$nahoda  )) 
                           {   
							if (mkdir($koren.$nick."/".$nahoda."/uploads/" ))
									{   if (mkdir($koren.$nick."/".$nahoda."/uploads/".$target_dir))
...
if( $_SESSION['vysledek'] == "vytvořeno"  ) //vložení nové kapely do databáze
	...
	$heslo = strtolower($heslo);
    $over_heslo = mysql_real_escape_string($_POST['over_heslo']);
	$over_heslo = strtolower($over_heslo);
    $md5_heslo = md5($heslo);
