...
module.exports = async ({ base, clients, good, request, upload, wav, db, check, temp }) => {
    ...
    try {
        ...
        const actionsDialog = page.locator('#recording-actions-' + ids[0]);
        ...
        await actionsDialog.getByRole('button', { name: 'Úplně smazat', exact: true }).waitFor();
        await actionsDialog.getByRole('button', { name: 'Kopírovat odkaz na čas', exact: true }).click();
        const link = await page.evaluate(() => navigator.clipboard.readText());
        assert.equal(new URL(link).searchParams.get('recording_id'), String(ids[0]));
        await menuToggle.click();
        await actionsDialog.getByRole('button', { name: 'Upravit', exact: true }).click();
        ...
        assert.equal(await actionsDialog.getByLabel('Cílová skladba nebo zkouška').count(), 0, 'Move destination is hidden from the action dialog until requested');
        await menuToggle.click();
        await actionsDialog.getByRole('button', { name: 'Přesunout', exact: true }).click();
        ...
        await guest.locator('#recording-' + ids[1] + ' .recording-actions').getByRole('button', { name: 'Další', exact: true }).click();
