...
2. Ve společné databázi jednou spusťte celý soubor
   `_pomocne/migrations/001_personal_accounts.sql`. Vytvoří jen `users`, `auth_settings`
...
`_pomocne/tests/auth_integration.php` se spouští pouze přes PHP CLI s rozšířením `mysqli`.
...
php _pomocne/tests/auth_integration.php
```
